...
def compute_AUC(alpha, data_path, data_name, overlap, repeat_index=0):
	...
	ent_file_name = 'fix-intra_semi_embeddings/' + '%s_MMD_WARM-START_SEMI%s_all_ent_embedding_alpha=%s_lamda=%s_semi=%s_repeat=%s.pt' %(data_name, overlap, ALPHA, LAMDA, inter_data_prop, repeat_index)
	rel_file_name = 'fix-intra_semi_embeddings/' + '%s_MMD_WARM-START_SEMI%s_all_rel_embedding_alpha=%s_lamda=%s_semi=%s_repeat=%s.pt' %(data_name, overlap, ALPHA, LAMDA, inter_data_prop, repeat_index)
...
if __name__ == '__main__':
	...
	path_dict = {
	'-UNOVERLAPED': '../data/KG_datasets/%s/kg/divided/fix_intra-test/' ,
	'' : '../data/KG_datasets/%s/semi/divided/fix_intra-test/' ,
	'-3': '../data/KG_datasets/%s/semi_3/divided/fix_intra-test/' ,
	'-5': '../data/KG_datasets/%s/semi_5/divided/fix_intra-test/' ,
	}
	...
	with torch.cuda.device(0):
		...
		with open('fix-intra_semi_AUCs/%s_AUCs_MMD.txt' %(data_name), 'w') as file:
			for overlap in ['-UNOVERLAPED', '', '-3', '-5']:
				...
				for alpha in [5.]:
					...
					if alpha != 0:
						with open('best_hypers/%s_overlap=%s_best_hypers_MMD.pkl' %(data_name, overlap_percent), 'rb') as hyper_file:
							best_hypers = pickle.load(hyper_file).params
						
						alpha = best_hypers['alpha']


					data_path = path_dict[overlap] %data_name.lower()
