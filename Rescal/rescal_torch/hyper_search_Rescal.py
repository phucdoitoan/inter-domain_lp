...
def objective(trial, data_file, data_name, overlap):
	...
	# tuning hyper
	alpha = trial.suggest_categorical('alpha', [0.0])
...
if __name__ == '__main__':
	...
	with torch.cuda.device(1):
		...
		for data_file, overlap in data_file_dict[data_name]:
			...
			with open('best_hypers/%s_overlap=%s_best_hypers.pkl' %(data_name, overlap), 'wb') as file:
				pickle.dump(trial, file)

			with open('best_hypers/%s_overlap=%s_best_hypers.txt' %(data_name, overlap), 'w') as file:
