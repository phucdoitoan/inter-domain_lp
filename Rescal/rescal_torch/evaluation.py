...
# copied and modified from https://github.com/torchkge-team/torchkge/blob/master/torchkge/evaluation/link_prediction.py
...
class LinkPredictionEvaluator(object):
    ...
    * Antoine Bordes, Nicolas Usunier, Alberto Garcia-Duran, Jason Weston,
    ...
    def evaluate(self, b_size, k_max, verbose=True):
        ...
        self.rank_true_heads = self.rank_true_heads.to(self.device)
        self.rank_true_tails = self.rank_true_tails.to(self.device)
        self.filt_rank_true_heads = self.filt_rank_true_heads.to(self.device)
        self.filt_rank_true_tails = self.filt_rank_true_tails.to(self.device)
    ...
    def mean_rank(self):
        ...
        if not self.evaluated:
            raise NotimplementedError('NotYetEvaluatedError: Evaluator not evaluated call '
