...
data_name = 'WN18RR'
path = '../data/%s/semi/divided/fix_intra-test/' %data_name
...
def save(batch_ot_loss_list, epoch_ot_loss_list, epoch_aveg_loss_list, data_name, ALPHA, LAMDA, inter_data_prop, semi_model, intra_kg_test, inter_kg_test, inter_kg_valid, repeat_index, savefig=False, test=False):
	...
	if savefig:
		...
		plt.savefig('fix-intra_semi_plots/%s_WARM-START_SEMI_batch_ot_loss_alpha=%s_lamda=%s_semi=%s.png' %(data_name, ALPHA, LAMDA, inter_data_prop))
		plt.clf()
		...
		plt.savefig('fix-intra_semi_plots/%s_WARM-START_SEMI_epoch_ot_loss_alpha=%s_lamda=%s_semi=%s.png' %(data_name, ALPHA, LAMDA, inter_data_prop))
		plt.clf()
		...
		plt.savefig('fix-intra_semi_plots/%s_WARM-START_SEMI_running_loss_alpha=%s_lamda=%s_semi=%s.png' %(data_name, ALPHA, LAMDA, inter_data_prop))
		plt.clf()

		with open('fix-intra_semi_plots/%s_WARM-START_SEMI_every-loss_alpha=%s_lamda=%s_semi=%s.pkl' %(data_name, ALPHA, LAMDA, inter_data_prop), 'wb') as file:
			loss_dict = {
	...
	if test:
		...
		semi_model.ent_emb.weight.data = load_embedding('fix-intra_semi_embeddings/' + '%s_WARM-START_SEMI_all_ent_embedding_alpha=%s_lamda=%s_semi=%s_repeat=%s.pt' %(data_name, ALPHA, LAMDA, inter_data_prop, repeat_index))
		semi_model.rel_mat.weight.data = load_embedding('fix-intra_semi_embeddings/' + '%s_WARM-START_SEMI_all_rel_embedding_alpha=%s_lamda=%s_semi=%s_repeat=%s.pt' %(data_name, ALPHA, LAMDA, inter_data_prop, repeat_index))
...
def save_embedding(path, ent_embedding, rel_embedding, n1, n_common, data_name, ALPHA, LAMDA, inter_data_prop, repeat_index):
	...
	#torch.save(ent_embedding[:n1 + n_common], path + '%s_WARM-START_SEMI_embedding_1_alpha=%s_lamda=%s_semi=%s_repeat=%s.pt' %(data_name, ALPHA, LAMDA, inter_data_prop, repeat_index))
	#torch.save(ent_embedding[n1:], path + '%s_WARM-START_SEMI_embedding_2_alpha=%s_lamda=%s_semi=%s_repeat=%s.pt' %(data_name, ALPHA, LAMDA, inter_data_prop, repeat_index))
	
	torch.save(ent_embedding, path + '%s_WARM-START_SEMI_all_ent_embedding_alpha=%s_lamda=%s_semi=%s_repeat=%s.pt' %(data_name, ALPHA, LAMDA, inter_data_prop, repeat_index))
	torch.save(rel_embedding, path + '%s_WARM-START_SEMI_all_rel_embedding_alpha=%s_lamda=%s_semi=%s_repeat=%s.pt' %(data_name, ALPHA, LAMDA, inter_data_prop, repeat_index))
...
with torch.cuda.device(1):
	for inter_prop in [0.0]: #[0.1, 0.3]:
		...
		with open('fix-intra_semi_results/%s_WARM-START_SEMI_AVERAGE_alpha=%s_lamda=%s_semi=%s.txt' % (data_name, al, LAMDA, inter_prop), 'w') as file:
