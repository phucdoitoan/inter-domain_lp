...
class Semi_Rescal(Rescal):
	...
	def __init__(self, emb_dim, n_ent_list, n_rel, alpha, device, init_ent_emb, init_rel_emb, use_MMD=False, MMD_kernel_mu=None, **kwargs): # tuning_mu
		...
		if not self.use_MMD:
			...
			print('The bandwith in MMD kernel is mu = %.4f' %MMD_kernel_mu)
			self.mmd = MMD_loss(MMD_kernel_mu)  # tuning mu
